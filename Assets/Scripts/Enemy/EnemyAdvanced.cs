...
public class EnemyAdvanced : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Target == null)
        {
            if (timethink <= 0)
            {
                targetPosition = new Vector3(Random.Range(-PatolDistance, PatolDistance), 0,
                    Random.Range(-PatolDistance, PatolDistance));
                    ...
                    if (precheck.collider && precheck.collider.CompareTag("Player"))
                    {
                        if (Random.Range(0, 99) < 100 * ShootRate)
                        {
                            if (Time.time > shoottemp + ShootDelay)
                            {
                                ...
                                Vector3 spread = new Vector3(Random.Range(-ShootSpread, ShootSpread),
                                                     Random.Range(-ShootSpread, ShootSpread),
                                                     Random.Range(-ShootSpread, ShootSpread)) /
                                                 100;
                                Vector3 dir = ((Target.transform.position + spread - (Vector3.up * 1f)) -
    ...
    void ShootTarget(Vector3 position, Vector3 direction)
    {
        ...
        {
            GameObject bullet =
                (GameObject)GameObject.Instantiate(Projectile.gameObject, position, this.transform.rotation);
